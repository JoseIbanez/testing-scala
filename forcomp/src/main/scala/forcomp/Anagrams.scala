...
object Anagrams extends AnagramsInterface {
   ...
   */
  def wordOccurrences(w: Word): Occurrences =  w.toList.groupBy( e => e.toLower ).map(kv => (kv._1, kv._2.length)).toList.sortBy(kv => kv._1)


  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences = s.flatMap(w => wordOccurrences(w)).sortBy(kv => kv._1)
